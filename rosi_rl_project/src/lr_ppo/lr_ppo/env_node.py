...
from lr_ppo.observation import ObservationBuilder
from lr_ppo.reward import RewardFunction
from lr_ppo.termination import TerminationChecker
...
# --------------------
        ...
        if self.latest_action is not None:
            linear = float(self.latest_action[0]) * 2.0
            angular = float(self.latest_action[1]) * 1.5

        # --------------------
        # Safety clamp (DO NOT DISABLE)
        # --------------------
...
# --------------------
        ...
        done_env, env_reason = self.termination_checker.check(
            scan_msg=self.scan,
            goal_distance=self.goal_distance,
        )
        ...
        self.get_logger().info(
            f"reward={reward:.3f} | min_range={min_range:.2f} | "
            f"action={self.latest_action} | done={done}"
        )
